...
# ifdef CYGFUN_LIBC_TIME_POSIX
#  define gmtime_r(__timer, __result) __gmtime_r(__timer, __result)
# endif
...
#include <sys/timeutil.h>        // for cyg_libc_time_normalize_structtm()
...
__localtime_r( const time_t *__timer, struct tm *__result )
{
    ...
    CYG_REPORT_FUNCNAMETYPE("__localtime_r", "returning %08x");
    CYG_CHECK_DATA_PTR(__timer, "__timer is not a valid pointer!");
    CYG_CHECK_DATA_PTR(__result, "__result is not a valid pointer!");
    CYG_REPORT_FUNCARG2("*__timer=%d, __result is at %08x",
                        *__timer, __result);
...
# ifdef CYGFUN_LIBC_TIME_POSIX
#  define localtime_r(__timer, __result) __localtime_r(__timer, __result)
# endif
...
__ctime_r( const time_t *__timer, char *__buf )
{
    ...
    CYG_REPORT_FUNCNAMETYPE("__ctime_r", "returning \"%s\"");
...
# ifdef CYGFUN_LIBC_TIME_POSIX
#  define ctime_r(__timer, __buf) __ctime_r(__timer, __buf)
# endif
...
__difftime( time_t __time1, time_t __time0 )
{
    double _ret;
    
    CYG_REPORT_FUNCNAMETYPE("__difftime", "returning %f");
...
# define difftime(__time1, __time0) __difftime(__time1, __time0)
...
__mktime( struct tm *__timeptr )
{
    ...
    CYG_REPORT_FUNCNAMETYPE("__mktime", "returning %d");
...
# define mktime(__timeptr) __mktime(__timeptr)
...
__asctime( const struct tm *__timeptr )
{
...
# define asctime(__timeptr) __asctime(__timeptr)
...
__gmtime( const time_t *__timer )
{
    CYG_REPORT_FUNCNAMETYPE("__gmtime", "returning %08x");
    CYG_CHECK_DATA_PTR(__timer, "__timer is not a valid pointer!");
...
# define gmtime(__timer) __gmtime(__timer)
...
__localtime( const time_t *__timer )
{
    CYG_REPORT_FUNCNAMETYPE("__localtime", "returning %08x");
    CYG_CHECK_DATA_PTR(__timer, "__timer is not a valid pointer!");
    CYG_REPORT_FUNCARG1("*__timer=%d", *__timer);
...
# define localtime(__timer) __localtime(__timer)
...
__ctime( const time_t *__timer )
{
    char *_str;

    CYG_REPORT_FUNCNAMETYPE("__ctime", "returning \"%s\"");
    CYG_CHECK_DATA_PTR( __timer, "__timer is not a valid pointer!");
    CYG_REPORT_FUNCARG1("*__timer = %d", *__timer);

    _str = __asctime(__localtime(__timer));
...
# define ctime(__timer) __ctime(__timer)
