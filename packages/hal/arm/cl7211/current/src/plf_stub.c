...
#if CYGHWR_HAL_ARM_CL7211_GDB_PORT == 0
...
cyg_hal_gdb_isr(cyg_vector_t vector, cyg_addrword_t data, HAL_SavedRegisters *regs)
{
    ...
    c = hal_cl7211_get_char();  // Fetch the character
    if( 3 == c ) {  // ^C
...
int 
hal_cl7211_interruptible(int state)
{
    if (state) {
...
void hal_cl7211_put_char(int c)
{
    cyg_uint32 stat;
